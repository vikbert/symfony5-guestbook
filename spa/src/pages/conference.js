...
function Comment({ comments }) {
    ...
    if (comments !== null && comments.length === 0) {
        return <div className="pt-4 text-center">No comments yet</div>;
    }

    if (comments === null) {
        return <div className="pt-4 text-center">loading ...</div>
    }
                    ...
                    <div className="comment-img mr-3">
                        {!comment.photoFilename ? '' : (
                            <a href={ENV_API_ENDPOINT + 'uploads/photos/' + comment.photoFilename} target="_blank">
                                <img src={ENV_API_ENDPOINT + 'uploads/photos/' + comment.photoFilename} alt="comment image"/>
                            </a>
                        )}
                    </div>
